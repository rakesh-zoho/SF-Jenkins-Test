...
- Location: tests\lead-creation.spec.js:129:3
...
Error: strict mode violation: getByLabel(/Company/i) resolved to 3 elements:
    ...
    3) <input required="" type="text" part="input" id="input-870" name="Company" maxlength="255" lwc-enmikoh2qu="" class="slds-input" aria-describedby="help-message-870"/> aka getByRole('textbox', { name: 'Company' })
...
> 168 |       await expect(companyInput).toBeVisible();
  ...
  193 |       await phoneInput.fill('(555) 123-4567');
  194 |       await expect(phoneInput).toHaveValue('(555) 123-4567');
  195 |     });
  196 | 
  197 |     // 9. Enter "San Francisco" in the City field
  198 |     await sfStep('Enter City: San Francisco', page, async () => {
  199 |       const cityInput = page.getByLabel(/City/i);
  200 |       await expect(cityInput).toBeVisible();
  201 |       await cityInput.fill('San Francisco');
  202 |       await expect(cityInput).toHaveValue('San Francisco');
  203 |     });
  204 | 
  205 |     // 10. Enter "CA" in the State field
  206 |     await sfStep('Enter State: CA', page, async () => {
  207 |       const stateInput = page.getByLabel(/State|Province/i);
  208 |       await expect(stateInput).toBeVisible();
  209 |       await stateInput.fill('CA');
  210 |       await expect(stateInput).toHaveValue('CA');
  211 |     });
  212 | 
  213 |     // 11. Enter "94105" in the Postal Code field
  214 |     await sfStep('Enter Postal Code: 94105', page, async () => {
  215 |       const postalInput = page.getByLabel(/Postal Code|Zip/i);
  216 |       await expect(postalInput).toBeVisible();
  217 |       await postalInput.fill('94105');
  218 |       await expect(postalInput).toHaveValue('94105');
  219 |     });
  220 | 
  221 |     // 12. Enter "United States" in the Country field
  222 |     await sfStep('Enter Country: United States', page, async () => {
  223 |       const countryInput = page.getByLabel(/Country/i);
  224 |       await expect(countryInput).toBeVisible();
  225 |       await countryInput.fill('United States');
  226 |       await expect(countryInput).toHaveValue('United States');
  227 |     });
  228 | 
  229 |     // 13. Select "Other" from the Lead Source dropdown
  230 |     await sfStep('Select Lead Source: Other', page, async () => {
  231 |       const leadSourceCombobox = page.getByLabel(/Lead Source/i);
  232 |       await expect(leadSourceCombobox).toBeVisible();
  233 |       await leadSourceCombobox.click();
  234 |       await page.getByRole('option', { name: /Other/i }).click();
  235 |       await expect(leadSourceCombobox).toContainText('Other');
  236 |     });
  237 | 
  238 |     // 14. Select "$10,000 to $50,000" from the Annual Revenue dropdown
  239 |     await sfStep('Select Annual Revenue: $10,000 to $50,000', page, async () => {
  240 |       const revenueCombobox = page.getByLabel(/Annual Revenue/i);
  241 |       await expect(revenueCombobox).toBeVisible();
  242 |       await revenueCombobox.click();
  243 |       await page.getByRole('option', { name: /\$10,000 to \$50,000/i }).click();
  244 |       await expect(revenueCombobox).toContainText(/10,000.*50,000/i);
  245 |     });
  246 | 
  247 |     // 15. Select "Purchased List" from the Lead Status dropdown
  248 |     await sfStep('Select Lead Status: Purchased List', page, async () => {
  249 |       const statusCombobox = page.getByLabel(/Lead Status/i);
