...
- Location: tests\lead-creation.spec.js:371:3
...
Locator: locator('[role="alert"]:has-text("Last Name")')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[role="alert"]:has-text("Last Name")')
...
```ts
  312 | 
  313 |   /**
  314 |    * Test 2.1: Attempt to Save Lead Without Required Fields
  315 |    * Verify that the system prevents saving a Lead when required fields are empty.
  316 |    */
  317 |   sfTest('Attempt to Save Lead Without Required Fields', async ({ sfPage: page }) => {
  318 |     // 1. Navigate to Leads app
  319 |     await sfStep('Navigate to Leads app', page, async () => {
  320 |       // HEALED: Using direct App Launcher navigation with specific locators
  321 |       await page.getByTitle('App Launcher').click({ timeout: 10000 });
  322 |       await page.waitForTimeout(500);
  323 |       await page.getByPlaceholder(/search/i).fill('Leads', { timeout: 5000 });
  ...
  327 |     });
  328 | 
  329 |     // 2. Click the "New" button on the Leads list view
  330 |     await sfStep('Click New button', page, async () => {
  331 |       await page.getByRole('button', { name: /^New$/i }).click();
  332 |       await waitForSFLoad(page);
  333 |       // HEALED: Form is in a dialog, not a plain form element
  334 |       await expect(page.getByRole('dialog')).toBeVisible({ timeout: 10000 });
  335 |     });
  336 | 
  337 |     // 3. Leave all fields empty and click the "Save" button
  338 |     await sfStep('Click Save with empty fields', page, async () => {
  339 |       await page.getByRole('button', { name: /^Save$/i }).click();
  340 |     });
  341 | 
  342 |     // Verify save action is prevented and error message appears
  343 |     await sfStep('Verify error message for missing required fields', page, async () => {
  344 |       // Should remain on the creation form
  345 |       // HEALED: Form is in a dialog, not a plain form element
  346 |       await expect(page.getByRole('dialog')).toBeVisible();
  347 | 
  348 |       // Error message should appear indicating required fields
  349 |       const errorMsg = page.locator('.form-error, .errorMessage, [role="alert"]:has-text("required")');
  350 |       await expect(errorMsg).toBeVisible({ timeout: 5000 });
  351 |     });
  352 | 
  353 |     // Verify fields are highlighted in red or with error indicator
  354 |     await sfStep('Verify required fields are highlighted', page, async () => {
  355 |       const firstNameField = page.getByLabel(/First Name/i);
  356 |       const lastNameField = page.getByLabel(/Last Name/i);
  357 |       const companyField = page.getByLabel(/Company/i);
  358 | 
  359 |       // Check for error styling (aria-invalid or error class)
  360 |       await expect(firstNameField.or(firstNameField.locator('..')))
  361 |         .toHaveAttribute(/class/i, /(error|invalid|required)/i).catch(() => {
  362 |           // Acceptable if attribute doesn't exist — visual error should still show
  363 |         });
  364 |     });
  365 |   });
  366 | 
  367 |   /**
  368 |    * Test 2.2: Enter Only First Name and Attempt Save
  369 |    * Verify validation catches missing required fields (Last Name).
  370 |    */
  371 |   sfTest('Enter Only First Name and Attempt Save', async ({ sfPage: page }) => {
  372 |     // 1. Navigate to Leads app
  373 |     await sfStep('Navigate to Leads app', page, async () => {
  374 |       // HEALED: Using direct App Launcher navigation with specific locators
  375 |       await page.getByTitle('App Launcher').click({ timeout: 10000 });
  376 |       await page.waitForTimeout(500);
  377 |       await page.getByPlaceholder(/search/i).fill('Leads', { timeout: 5000 });
  378 |       await page.waitForTimeout(500);
  379 |       await page.getByRole('option', { name: /^Leads$/i }).click({ timeout: 5000 });
  380 |       await waitForSFLoad(page);
  381 |     });
  382 | 
  383 |     // 2. Click the "New" button on the Leads list view
  384 |     await sfStep('Click New button', page, async () => {
  385 |       await page.getByRole('button', { name: /^New$/i }).click();
  386 |       await waitForSFLoad(page);
  387 |       // HEALED: Form is in a dialog, not a plain form element
  388 |       await expect(page.getByRole('dialog')).toBeVisible({ timeout: 10000 });
  389 |     });
  390 | 
  391 |     // 3. Enter "Michael" in the First Name field
  392 |     await sfStep('Enter First Name: Michael', page, async () => {
  393 |       const firstNameInput = page.getByLabel(/First Name/i);
  394 |       await expect(firstNameInput).toBeVisible();
  395 |       await firstNameInput.fill('Michael');
  396 |       await expect(firstNameInput).toHaveValue('Michael');
  397 |     });
  398 | 
  399 |     // 4. Leave Last Name empty and click the "Save" button
  400 |     await sfStep('Click Save with missing Last Name', page, async () => {
  401 |       await page.getByRole('button', { name: /^Save$/i }).click();
  402 |     });
  403 | 
  404 |     // Verify save action fails and error is displayed
  405 |     await sfStep('Verify error for missing Last Name', page, async () => {
  406 |       // Should remain on the creation form
  407 |       // HEALED: Form is in a dialog, not a plain form element
  408 |       await expect(page.getByRole('dialog')).toBeVisible();
  409 | 
  410 |       // Error message should appear indicating missing Last Name
  411 |       const errorMsg = page.locator('[role="alert"]:has-text("Last Name")');
> 412 |       await expect(errorMsg).toBeVisible({ timeout: 5000 });
      |                              ^ Error: expect(locator).toBeVisible() failed
  413 |     });
  414 | 
  415 |     // Verify First Name is retained in the form
  416 |     await sfStep('Verify First Name is retained', page, async () => {
  417 |       const firstNameInput = page.getByLabel(/First Name/i);
  418 |       await expect(firstNameInput).toHaveValue('Michael');
  419 |     });
  420 |   });
  421 | 
  422 |   /**
  423 |    * Test 2.3: Enter Only Last Name and Company and Attempt Save
  424 |    * Verify that First Name is required even when other fields are filled.
  425 |    */
  426 |   sfTest('Enter Only Last Name and Company and Attempt Save', async ({ sfPage: page }) => {
  427 |     // 1. Navigate to Leads app
  428 |     await sfStep('Navigate to Leads app', page, async () => {
  429 |       // HEALED: Using direct App Launcher navigation with specific locators
  430 |       await page.getByTitle('App Launcher').click({ timeout: 10000 });
  431 |       await page.waitForTimeout(500);
  432 |       await page.getByPlaceholder(/search/i).fill('Leads', { timeout: 5000 });
  433 |       await page.waitForTimeout(500);
  434 |       await page.getByRole('option', { name: /^Leads$/i }).click({ timeout: 5000 });
  435 |       await waitForSFLoad(page);
  436 |     });
  437 | 
  438 |     // 2. Click the "New" button on the Leads list view
  439 |     await sfStep('Click New button', page, async () => {
  440 |       await page.getByRole('button', { name: /^New$/i }).click();
  441 |       await waitForSFLoad(page);
  442 |       // HEALED: Form is in a dialog, not a plain form element
  443 |       await expect(page.getByRole('dialog')).toBeVisible({ timeout: 10000 });
  444 |     });
  445 | 
  446 |     // 3. Leave First Name empty
  447 |     // 4. Enter "Johnson" in the Last Name field
  448 |     await sfStep('Enter Last Name: Johnson', page, async () => {
  449 |       const lastNameInput = page.getByLabel(/Last Name/i);
  450 |       await expect(lastNameInput).toBeVisible();
  451 |       await lastNameInput.fill('Johnson');
  452 |       await expect(lastNameInput).toHaveValue('Johnson');
  453 |     });
  454 | 
  455 |     // 5. Enter "Global Enterprises" in the Company field
  456 |     await sfStep('Enter Company: Global Enterprises', page, async () => {
  457 |       const companyInput = page.getByLabel(/Company/i);
  458 |       await expect(companyInput).toBeVisible();
  459 |       await companyInput.fill('Global Enterprises');
  460 |       await expect(companyInput).toHaveValue('Global Enterprises');
  461 |     });
  462 | 
  463 |     // 6. Click the "Save" button
  464 |     await sfStep('Click Save with missing First Name', page, async () => {
  465 |       await page.getByRole('button', { name: /^Save$/i }).click();
  466 |     });
  467 | 
  468 |     // Verify save fails and error indicates First Name is required
  469 |     await sfStep('Verify error for missing First Name', page, async () => {
  470 |       // Should remain on the creation form
  471 |       // HEALED: Form is in a dialog, not a plain form element
  472 |       await expect(page.getByRole('dialog')).toBeVisible();
  473 | 
  474 |       // Error message should indicate First Name is required
  475 |       const errorMsg = page.locator('[role="alert"]:has-text("First Name")');
  476 |       await expect(errorMsg).toBeVisible({ timeout: 5000 });
  477 |     });
  478 | 
  479 |     // Verify user can correct and save
  480 |     await sfStep('Verify field values are retained and ready to correct', page, async () => {
  481 |       const lastNameInput = page.getByLabel(/Last Name/i);
  482 |       const companyInput = page.getByLabel(/Company/i);
  483 |       await expect(lastNameInput).toHaveValue('Johnson');
  484 |       await expect(companyInput).toHaveValue('Global Enterprises');
  485 |     });
  486 |   });
  487 | 
  488 |   /**
  489 |    * Test 2.4: Enter Invalid Email Format
  490 |    * Verify email field validation for proper format.
  491 |    */
  492 |   sfTest('Enter Invalid Email Format', async ({ sfPage: page }) => {
  493 |     // 1. Navigate to Leads app
  494 |     await sfStep('Navigate to Leads app', page, async () => {
  495 |       // HEALED: Using direct App Launcher navigation with specific locators
  496 |       await page.getByTitle('App Launcher').click({ timeout: 10000 });
  497 |       await page.waitForTimeout(500);
  498 |       await page.getByPlaceholder(/search/i).fill('Leads', { timeout: 5000 });
  499 |       await page.waitForTimeout(500);
  500 |       await page.getByRole('option', { name: /^Leads$/i }).click({ timeout: 5000 });
  501 |       await waitForSFLoad(page);
  502 |     });
  503 | 
  504 |     // 2. Click the "New" button on the Leads list view
  505 |     await sfStep('Click New button', page, async () => {
  506 |       await page.getByRole('button', { name: /^New$/i }).click();
  507 |       await waitForSFLoad(page);
  508 |       // HEALED: Form is in a dialog, not a plain form element
  509 |       await expect(page.getByRole('dialog')).toBeVisible({ timeout: 10000 });
  510 |     });
  511 | 
  512 |     // 3. Enter "Sarah" in the First Name field
```