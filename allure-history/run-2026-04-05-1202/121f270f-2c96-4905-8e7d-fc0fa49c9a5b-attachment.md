...
- Name: lead-creation.spec.js >> Lead Creation >> Enter Only Last Name and Company and Attempt Save
- Location: tests\lead-creation.spec.js:426:3
                            ...
                            - group "Name required" [ref=e1922]:
                              ...
                              - generic [ref=e1926]:
                                ...
                                - generic [ref=e1948]:
                                  ...
                                  - textbox "First Name" [ref=e1951]
                                - generic [ref=e1955]:
                                  - generic [ref=e1956]: "*Last Name"
                                  - textbox "Last Name" [active] [ref=e1958]: Johnson
                          - listitem [ref=e1960]:
                            - generic [ref=e1965]:
...
> 458 |       await expect(companyInput).toBeVisible();
  ...
  513 |     await sfStep('Enter First Name: Sarah', page, async () => {
  514 |       const firstNameInput = page.getByLabel(/First Name/i);
  515 |       await expect(firstNameInput).toBeVisible();
  516 |       await firstNameInput.fill('Sarah');
  517 |       await expect(firstNameInput).toHaveValue('Sarah');
  518 |     });
  519 | 
  520 |     // 4. Enter "Williams" in the Last Name field
  521 |     await sfStep('Enter Last Name: Williams', page, async () => {
  522 |       const lastNameInput = page.getByLabel(/Last Name/i);
  523 |       await expect(lastNameInput).toBeVisible();
  524 |       await lastNameInput.fill('Williams');
  525 |       await expect(lastNameInput).toHaveValue('Williams');
  526 |     });
  527 | 
  528 |     // 5. Enter "Digital Solutions" in the Company field
  529 |     await sfStep('Enter Company: Digital Solutions', page, async () => {
  530 |       const companyInput = page.getByLabel(/Company/i);
  531 |       await expect(companyInput).toBeVisible();
  532 |       await companyInput.fill('Digital Solutions');
  533 |       await expect(companyInput).toHaveValue('Digital Solutions');
  534 |     });
  535 | 
  536 |     // 6. Enter "invalid-email-format" in the Email field (missing @ symbol)
  537 |     await sfStep('Enter invalid email: invalid-email-format', page, async () => {
  538 |       const emailInput = page.getByLabel(/Email/i);
  539 |       await expect(emailInput).toBeVisible();
  540 |       await emailInput.fill('invalid-email-format');
  541 |       await expect(emailInput).toHaveValue('invalid-email-format');
  542 |     });
  543 | 
  544 |     // 7. Click the "Save" button
  545 |     await sfStep('Click Save with invalid email', page, async () => {
  546 |       await page.getByRole('button', { name: /^Save$/i }).click();
  547 |     });
  548 | 
  549 |     // Verify email validation
  550 |     await sfStep('Verify email format validation is enforced', page, async () => {
  551 |       // Either save is prevented with email validation error, OR
  552 |       // Lead saves but email field shows validation warning
  553 |       // HEALED: Form is in a dialog, not a plain form element
  554 |       const emailErrorMsg = page.locator('[role="alert"]:has-text("email")');
  555 |       const stillOnForm = page.getByRole('dialog').isVisible();
  556 | 
  557 |       const hasError = await emailErrorMsg.isVisible().catch(() => false);
  558 |       const onFormStill = await stillOnForm;
```