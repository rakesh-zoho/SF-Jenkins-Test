...
- Location: tests\lead-creation.spec.js:303:3
...
```ts
  ...
  231 |     });
  232 | 
  233 |     // 15. Select "Purchased List" from the Lead Status dropdown
  234 |     await sfStep('Select Lead Status: Purchased List', page, async () => {
  235 |       const statusCombobox = page.getByLabel(/Lead Status/i);
  236 |       await expect(statusCombobox).toBeVisible();
  237 |       await statusCombobox.click();
  238 |       await page.getByRole('option', { name: /Purchased List/i }).click();
  239 |       await expect(statusCombobox).toContainText('Purchased List');
  240 |     });
  241 | 
  242 |     // 16. Select "Warm" from the Rating dropdown
  243 |     await sfStep('Select Rating: Warm', page, async () => {
  244 |       const ratingCombobox = page.getByLabel(/Rating/i);
  245 |       await expect(ratingCombobox).toBeVisible();
  246 |       await ratingCombobox.click();
  247 |       await page.getByRole('option', { name: /Warm/i }).click();
  248 |       await expect(ratingCombobox).toContainText('Warm');
  249 |     });
  250 | 
  251 |     // 17. Enter "High priority prospect for Q2" in the Description field
  252 |     await sfStep('Enter Description: High priority prospect for Q2', page, async () => {
  253 |       const descriptionInput = page.getByLabel(/Description/i);
  254 |       await expect(descriptionInput).toBeVisible();
  255 |       await descriptionInput.fill('High priority prospect for Q2');
  256 |       await expect(descriptionInput).toHaveValue('High priority prospect for Q2');
  257 |     });
  258 | 
